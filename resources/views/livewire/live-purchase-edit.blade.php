<x-modal wire:model="showPurchaseEditModal" size="xxl" cancelButtonLabel="Close">
    @if ($editPurchaseNow)
        <x-slot name="modalHeader">
            <div class="flex space-x-3 items-baseline">
                <h1 class="text-2xl font-semibold">Edit a Purchase</h1>
                                            ...
                                            <thead class="bg-gray-50">
                                                <tr class="divide-x">
                                                    ...
                                                    <x-th align="center" width="100px">
                                                        <button wire:click="$emit('searchAddItem', '{{ $purchase_id }}')" type="button"
                                                ...
                                                @isset($purchaseDetails)
                                                    @foreach ($purchaseDetails as $key => $pd)
                                                        <tr
                                                            ...
                                                            <x-td-centered-slim class="text-center">
                                                                {{ number_format($pd['qty'] * $pd['price'], 0, '.', ',') }}
                                                            </x-td-centered-slim>
        ...
        <x-slot name="modalAction">
            <button {{ $isDirty ? '' : 'disabled' }} type="button" wire:click="update"
