    ...
    <x-sticky-table-wrapper>
        <thead class="bg-gray-50">
            <tr class="divide-x">
                ...
                <x-th width="100px" align="center">Reg. No</x-th>
                <x-th>Room</x-th>
                <x-th>Date</x-th>
                <x-th>Name</x-th>
                <x-th width="100px" align="center">Sessions</x-th>
                <x-th align="center" width="120px">Rate</x-th>
                <x-th align="center" width="120px">Commission</x-th>
                <x-th align="center" width="120px">Commission Amount</x-th>
            </tr>
        </thead>
        <tbody class="bg-white">
            @foreach ($inhouseServices as $index => $detail)
                <tr class="divide-x hover:bg-gray-200 transition-colors ease-out duration-150 cursor-pointer">
                    ...
                    <x-td>{{ $detail->operation_date }}</x-td>
                    <x-td>{{ $detail->serviceStaff->nick_name }}</x-td>
                    <x-td-slim-with-align align="center">{{ $detail->session_hours }}</x-td-slim-with-align>
                    ...
                    <x-td-slim-with-align align="right">
                        {{ number_format($commission * $detail->session_hours, 0, '.', ',') }}
                    </x-td-slim-with-align>
                </tr>
            @endforeach
        </tbody>
